...
class Subtitles_Admin extends Subtitles {
	...
	public function build_subtitle_input( $post ) {
		...
		wp_nonce_field( // nonces ("number used once") are used to protect the custom meta box forms from being misused.
		...
		<div id="subtitlediv">
			<div id="subtitlewrap">
				<label class="screen-reader-text" id="subtitle-prompt-text" for="<?php echo self::SUBTITLE_META_KEY; ?>">
					<?php echo apply_filters( 'enter_subtitle_here', __( 'Enter subtitle here', 'subtitles' ), $post ); ?>
				</label><!-- #subtitle-prompt-text -->
				<input type="text" name="<?php echo self::SUBTITLE_META_KEY; ?>" size="30" value="<?php echo esc_attr( htmlspecialchars( $subtitle ) ); ?>" id="subtitle" autocomplete="off" />
			</div><!-- #subtitlewrap -->
		</div><!-- #subtitlediv --><?php
	...
	public function update_subtitle_data( $post_id, $post, $update ) {
		 ...
		 */
		if ( $is_nonce_set ) {
			$is_verified_nonce = (bool) wp_verify_nonce( $_POST[ self::SUBTITLE_NONCE_NAME ], basename( __FILE__ ) );
		}
		else {
